...
LOAD_TEMPLATE_PAGE('order_page', ORDER_CAPTION_CART);
INSERT_TEMPLATE_BLOCK(ORDER_BLOCK_SECTION_LIST());

$block = RET_TEMPLATE_CONTENT('order_cart');
...
$ra = array(
	'cart_total'		=> sprintf(ORDER_CART_TOTAL, RET_CART_TOTAL())
	...
	if ($cart_rows_count > 0) {
		...
		while ($i < $cart_rows_count) {
			...
			$ra[] = array(
				...
				,'remove_confirm'	=> STR_TO_JS(sprintf(ORDER_CART_REMOVE_ITEM_CONFIRM, $row[LIST_MEAL_NAME]))
			);
			++$i;
		}
	}
