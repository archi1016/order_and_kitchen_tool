...
if (LOAD_DB($the_db, $rows, $rows_count)) {
	...
	if (COUNT_PAGES_INFO($rows_count, 20, $THIS_WEB_URL, $pi)) {
		...
		$block = RET_TEMPLATE_CONTENT('back_flavor_list');
		if (MATCH_TEMPLATE_LIST_ITEM('flavor', $block, $key, $item, $ra)) {
			...
			while ($i <= $pi[PAGE_LAST]) {
				...
				$ra[] = array(
					'row_class'		=> ($r&1)
					,'row_no'		=> ($i+1)
					,'name'			=> STR_TO_HTML($row[FLAVOR_NAME])
					...
					,'remove_confirm'	=> STR_TO_JS(sprintf(BACK_ITEM_REMOVE_CONFIRM, STR_TO_HTML($row[FLAVOR_NAME])))
				);
				++$i;
				++$r;
			}
