...
$the_db = '__front/front_password.php';
require($the_db);

if (OPERATION_SAVE == $the_operation) {
	if (FRONT_ACCOUNT == RET_STR_POST('OLD_ACCOUNT')) {
		if (FRONT_PASSWORD == RET_STR_POST('OLD_PASSWORD')) {
			...
			$ls[] = '<'.'?php';
			$ls[] = '';
			$ls[] = RET_DEFINE_STRING('FRONT_ACCOUNT', $new_acc);
			$ls[] = RET_DEFINE_STRING('FRONT_PASSWORD', $new_pwd);
	...
	if (OPERATION_DONE == $the_operation) {
		...
		$ra = array(
			'msg' => BACK_CONFIG_FRONT_USER_MODIFY_DONE
		);
	...
	$block = RET_TEMPLATE_CONTENT('back_config_front_user');
	$ra = array(
		'action_title'			=> sprintf(BACK_TITLE_MODIFY, BACK_CONFIG_SECTION_FRONT_USER)
