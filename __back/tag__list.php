...
if (LOAD_DB($the_db, $rows, $rows_count)) {
	...
	if (COUNT_PAGES_INFO($rows_count, 20, $THIS_WEB_URL, $pi)) {
		...
		$block = RET_TEMPLATE_CONTENT('back_tag_list');
		if (MATCH_TEMPLATE_LIST_ITEM('tag', $block, $key, $item, $ra)) {
			$i = $pi[PAGE_OFFSET];
			$r = 0;
			while ($i <= $pi[PAGE_LAST]) {
				...
				$ra[] = array(
					'row_class'		=> ($r&1)
					,'row_no'		=> ($i+1)
					,'name'			=> STR_TO_HTML($row[TAG_NAME].RET_ITEM_EN($row[TAG_NAME_EN]))
					,'photo'		=> RET_PHOTO_FILE($the_folder, $row[TAG_ID], $row[TAG_PHOTO])
					,'hide_class'		=> RET_HIDE_PREFIX($row[TAG_HIDE])
					...
					,'remove_confirm'	=> STR_TO_JS(sprintf(BACK_ITEM_REMOVE_CONFIRM, STR_TO_HTML($row[TAG_NAME])))
				);
				++$i;
				++$r;
			}
