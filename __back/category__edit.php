...
$block = RET_TEMPLATE_CONTENT('back_category_edit');
$ra = array(
	'action_title'			=> STR_TO_HTML(sprintf(BACK_TITLE_MODIFY, $the_title))
	,'action_url'			=> $THIS_WEB_URL
	,'source_id'			=> $row[CATEGORY_ID]
	,'source_photo'			=> RET_PHOTO_FILE($the_folder, $row[CATEGORY_ID], $row[CATEGORY_PHOTO])
	,'source_name'			=> STR_TO_INPUT_VALUE($row[CATEGORY_NAME])
	,'source_name_english'		=> STR_TO_INPUT_VALUE($row[CATEGORY_NAME_EN])
	,'options_hide'			=> RET_OPTIONS_BOOLEAN($row[CATEGORY_HIDE])
	,'options_hide_in_order'	=> RET_OPTIONS_BOOLEAN($row[CATEGORY_HIDE_IN_ORDER])
);
