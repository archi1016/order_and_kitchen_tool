...
if (LOAD_DB($the_db, $rows, $rows_count)) {
	...
	if (COUNT_PAGES_INFO($rows_count, 50, $THIS_WEB_URL, $pi)) {
		APEND_URL_ARGUMENT($THIS_WEB_URL, ARGUMENT_PAGE, $pi[PAGE_CURRENT]);
		$block = RET_TEMPLATE_CONTENT('back_table_list');
		if (MATCH_TEMPLATE_LIST_ITEM('table', $block, $key, $item, $ra)) {
			$i = $pi[PAGE_OFFSET];
			$r = 0;
			while ($i <= $pi[PAGE_LAST]) {
				...
				$name = STR_TO_HTML($row[TABLE_NAME]);
				
				$ra[] = array(
					'row_class'		=> ($r&1)
					,'row_no'		=> ($i+1)
					,'name'			=> $name
					,'address'		=> STR_TO_HTML($row[TABLE_ADDRESS])
					...
					,'remove_confirm'	=> STR_TO_JS(sprintf(BACK_ITEM_REMOVE_CONFIRM, $name))
				);
				++$i;
				++$r;
			}
