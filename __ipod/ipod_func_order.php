...
function IPOD_CHECK_CART_MEALS() {
	global $THIS_WEB_URL;
	global $tmp_id;
	global $cart_db;
	global $cart_rows;
...
function IPOD_BLOCK_ORDER_SECTION($prev_url) {
	global $page;
	global $tmp_row;
	...
	$section_info = array(
		array('',		IPOD_ORDER_SECTION_MENU)
		,array(PAGE_CART,	sprintf(IPOD_ORDER_SECTION_CART, $rows))
		,array(PAGE_CHECKOUT,	sprintf(IPOD_ORDER_SECTION_CHECKOUT, $total))
	);
	
	$block = RET_TEMPLATE_CONTENT('ipod_order_section');
	...
	$search_action = 'ipod_order.php?'.ARGUMENT_SESSION_TOKEN.'='.SESSION_TOKEN;
...
function IPOD_BLOCK_ORDER_FOOTER() {
	global $no_section_url;
	
	$block = RET_TEMPLATE_CONTENT('ipod_order_footer');
	
	$cancel_url = $no_section_url;
...
function ORDER_BLOCK_LIST_GROUPS() {
	...
	$block = RET_TEMPLATE_CONTENT('ipod_order_list_category');
...
function ORDER_BLOCK_LIST_CATEGORY_MEALS($t) {
	...
	$block = RET_TEMPLATE_CONTENT('ipod_order_list_meal');
	$ra = array(
...
function ORDER_BLOCK_MEAL_INFO($action) {
	...
	if ('1' == $row_meal[MEAL_PAUSED]) {
		$block = RET_TEMPLATE_CONTENT('ipod_order_meal_paused');
		$ra = array(
	...
	$block = RET_TEMPLATE_CONTENT('ipod_order_meal');
	$ra = array(
		'action_url'		=> $action
...
function ORDER_BLOCK_LIST_FLAVORS($flavor_id) {
	...
	$block = RET_TEMPLATE_CONTENT('ipod_order_list_flavor');
...
function ORDER_BLOCK_LIST_ADDITIONALS($additional_id) {
	...
	$block = RET_TEMPLATE_CONTENT('ipod_order_list_additional');
...
function ORDER_BLOCK_SEARCH_MEAL() {
	...
	$block = RET_TEMPLATE_CONTENT('ipod_order_list_meal');
	$ra = array(
		'category_name' => IPOD_MEAL_TITLE_SEARCH
	);
	$block = RET_REPLACED_TEMPLATE($block, $ra);
	if (MATCH_TEMPLATE_LIST_ITEM('meal', $block, $key, $item, $ra)) {
