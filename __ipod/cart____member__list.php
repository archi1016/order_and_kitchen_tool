...
$block = RET_TEMPLATE_CONTENT('ipod_order_cart');
...
$ra = array(
	'cart_total'		=> sprintf(IPOD_CART_TOTAL, RET_CART_TOTAL())
	...
	if ($cart_rows_count > 0) {
		...
		while ($i < $cart_rows_count) {
			...
			$ra[] = array(
				...
				,'remove_confirm'	=> STR_TO_JS(sprintf(IPOD_CART_REMOVE_ITEM_CONFIRM, $row[LIST_MEAL_NAME]))
			);
			++$i;
		}
	}
